    ...
    describe("Common Wallet Tests" + (testConfig.liteMode ? " (lite mode)" : ""), function() {
      ...
      it("Can create wallets with subaddress lookahead", async function() {
        ...
        try {
         ...
         await receiver.sync();
         assert(genUtils.compareBigInt((await receiver.getBalance()), new BigInteger("0")) > 0);
        } catch (e) {
          err = e;
        }
...
//            assert(GenUtils.compareBigInt(check.getReceivedAmount(), BigInt(0)) > 0);
      ...
      async function testSendToMultiple(numAccounts, numSubaddressesPerAccount, canSplit, sendAmountPerSubaddress, useJsConfig, subtractFeeFromDestinations) {
        ...
        if (Math.abs((sendAmount - (subtractFeeFromDestinations ? feeSum : BigInt(0)) - outgoingSum).toJSValue()) > SEND_MAX_DIFF) { // send amounts may be slightly different
